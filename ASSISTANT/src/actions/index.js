        ...
        return request.then(
            response => {
                if (response.status === 200 && response.data.estado.codigoEstado === 200) {
                    ...
                    return request
                        .then(response => {
                            // console.log('RESPONSE MENSAJE FORM::', response);
                            if (
                                response.status === 200 &&
                                response.data.estado.codigoEstado === 200
                            ) {
...
export function getSixbellOut(dispatch, data) {
    ...
    if (intervalGlobal) {
        // console.log('intervalGlobal', intervalGlobal)
        ASISTANT_INTERVAL_TIMER = 5000;
        interval = setInterval(function() {
            ...
            axios.post(urlApi, newData, {
                ...
                if (dataResponse.estado.codigoEstado === 200) {
                    ...
                    // console.log('dataResponse.end_conversation ', dataResponse.end_conversation);
